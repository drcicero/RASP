...
def add_ops(Class,apply_unary_op,apply_binary_op):
	...
	def addsetname(f,opname,rev):
		def f_with_setname(*a):
			...
			# always_display as True (under assumption it is normally being called by the user,
			# who must clearly be naming some variable they care about)
	...
	# skipping and, or, xor, which are bitwise and dont implement 'and' and 'or' but rather & and |
	# similarly skipping lshift, rshift cause who wants them
	# wish i had not, and, or primitives, but can accept that dont.
	# if people really want to do 'not' they can do '==False' instead, can do a little macro for it in the other sugar file or whatever
	...
	@addop("round")
	def __round__(self): # not sure if python will get upset if round doesnt return an actual int tbh... will have to check.
		return apply_unary_op(self,round)
	
	# defining floor, ceil, trunc showed up funny (green instead of blue), gonna go ahead and avoid
